...
In response to these problems, the semiconductor industry is actively looking for an atomic-level engineering approach capable of achieving high-selectivity simultaneously. One of the most promising techniques is atomic layer etching (ALE). To etch metallic copper, a specific variant, the plasma-thermal ALE process is used. It is composed of two self-limiting steps (\cref{fig:scheme}): 1) conversion of surface layers of copper to copper oxide under oxygen plasma and 2) reaction of the oxide layer with the etchant molecules (\textit{e.g.} \ch{HCOOH}) to form volatile complexes. While plasma is required to impart the inherent directionality from the electric field-accelerated ions to devices, the accelerated ions lead to relatively thick, non-self-limiting activated layers (compared with thermal ALE), preventing the process from achieving a finer grain of control over etch per cycle (EPC). 
...
% for thermal oxidaiton, these are the important numbers:
% 1. adsorption energy (by sites)
% 100:
% missing row
% 2. O2 dissocitation barrier
% 100: 0.1 eV, ref. Reactivity of diatomic molecules on Cu(100)
% 3. atomic adsorbate diffusion barrier
% 4. vacancy diffusion barrier

% 'Barriers' to reproducing nucleation and coalescence:
% 1. time may not be long enough to achieve the most favorable structure. In this case, a static approach/KMC approach is preferred. 
% 2. surface cell may not be large enough to accomodate these islands.
% 3. potential is not accurate. there is little reason to believe that this is the case.

% for this paper, we want to finish the plasma initial investigation quickly, because this is what the potential is trained for. For thermal oxidation,
% experimentally there's a dwell time of up to 30 minutes
